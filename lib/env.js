...
export function parseCustomFlags(args) {
  ...
  return { remaining, envFile, envName, dir, all, existing, project, savePinData };
}
...
export function buildEnv() {
  ...
  if (env.N8N_URL && !/^https?:\/\//.test(env.N8N_URL)) {
    console.warn(`Warning: N8N_API_URL missing protocol, assuming https:// (${env.N8N_URL})`);
    env.N8N_URL = `https://${env.N8N_URL}`;
  }
  return env;
}
