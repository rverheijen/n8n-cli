...
export function pullWorkflow(workflowId, env) {
  ...
  try {
    const { activeVersion: _av, shared: _sh, versionId: _vi, activeVersionId: _avi, versionCounter: _vc, triggerCount: _tc, ...workflow } = JSON.parse(result.stdout.toString());
    return workflow;
  } catch {
...
export function pullAllWorkflows(workflowsDir, envName, manifest, env, { existing = false, project = null } = {}) {
